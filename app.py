...
# ── Notion同期 ────────────────────────────────────────────────────────────
def sync_to_notion(title: str, url: str, hashtags: str, memo: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if url:
            props["URL"] = {"url": url}
        if hashtags:
            props["ハッシュタグ"] = {"rich_text": [{"text": {"content": hashtags[:2000]}}]}
        if memo:
            props["メモ"] = {"rich_text": [{"text": {"content": memo[:2000]}}]}
        today = datetime.now().strftime("%Y-%m-%d")
        props["投稿日時"] = {"date": {"start": today}}

        # notion-client v3 は data_source_id で pages.create
        notion.pages.create(parent={"database_id": db_id}, properties=props)
        return True, f"Notion に「{title}」を追加しました（ステータス: {status}）"
...
# ── STEP 3: ★ 構成案レビュー ─────────────────────────────────────────────
    ...
    st.success("構成案を確認・修正してから、執筆を開始してください。テキストエリアは直接編集できます。")
...
# ── STEP 6: X投稿文 ───────────────────────────────────────────────────────
    ...
    note_url = st.text_input(
        "NoteのURL（投稿後に入力してください / まだなら空欄でOK）",
    ...
    prompt = load_prompt(
        ...
        url=note_url or "[NoteのURLをここに入力]",
    )
    ...
    x_post = st.text_area(
        ...
        height=220,
        placeholder="Claude.aiが作成したX投稿文（3パターン）をここに貼り付け...",
    )
...
# ── STEP 7: ★ 最終レビュー・保存 ─────────────────────────────────────────
    ...
    if st.session_state.saved_path:
        ...
        st.write("")
        if st.button("🔄 新しい記事を作成する", type="primary", use_container_width=True):
            reset()
    else:
        st.success("記事とX投稿文を確認して保存してください。テキストエリアは直接編集できます。")
        ...
        tab_article, tab_x = st.tabs(["📄 記事本文（前半 ＋ 後半）", "🐦 X投稿文"])
        ...
        with tab_x:
            x_post_edited = st.text_area(
                "X投稿文",
                value=st.session_state.x_post,
                height=240,
                label_visibility="collapsed",
            )
            if st.session_state.note_url:
                st.caption(f"Note URL: {st.session_state.note_url}")
...
"""
                ...
                # Notion同期
                if sync_notion:
                    ok, msg = sync_to_notion(
                        title=article_title,
                        url=st.session_state.note_url,
                        hashtags=x_post_edited[:500] if x_post_edited else "",
